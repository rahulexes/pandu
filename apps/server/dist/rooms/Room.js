...
import { GamePhase, GameMode, SpecialPowerType, SpecialActionPhase, GameEventType, DECK_SIZE, INITIAL_VIEW_DURATION_MS, PEEK_DURATION_MS, DEFAULT_CARDS_DEALT, DEFAULT_INITIAL_VIEWABLE, DEFAULT_QUEEN_COUNT, } from '@pandu/shared';
import { GameStateMachine } from '../engine/GameStateMachine.js';
import { TurnSystem } from '../engine/TurnSystem.js';
import { createDeck, shuffleDeck, dealCards, drawFromPile, addToDiscardPile, recycleDiscardPile, getVisibleDiscards, } from '../engine/Deck.js';
...
import { validatePanduCall, createPanduState } from '../engine/PanduSystem.js';
...
export class Room {
    ...
    emitEvent;
    // ── Session tokens ──
    sessionTokens = new Map(); // sessionToken → playerId
    ...
    updateSettings(hostId, updates) {
        ...
        const actualUpdates = (updates?.settings ? updates.settings : updates);
    ...
    startGame() {
        ...
        let entities; // player IDs or team IDs
    ...
    placePenaltyCard(playerId, slotIndex) {
